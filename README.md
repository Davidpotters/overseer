...
A control-plane component that watches an AI system from outside it. It
isn't an agent itself: its whole job is to notice when a system under
watch is doing something it shouldn't, tell a human, and be able to
...
- **Two independent hash-chained logs, not one shared one.** Report
  writes its own alert log rather than appending into Monitor's. Two
  independent processes maintaining one hash chain in memory would
  silently race and corrupt it the moment they interleaved, a problem
  caught at design time, before any code shipped.
- **SIGKILL, not a graceful stop.** Halt calls the Docker Engine API's
  ...
  process can install a handler for and ignore. Confirmed live (see
  the Phase 4 adversarial test below) before deciding this mattered enough
  to hard-code.
...
- **CI:** GitHub Actions, running CodeQL, gitleaks, Trivy, Syft-based SBOM
