...
inert syscall attempt inside the sandbox proven in
Technical_Documentation/overseer/toy-agent-containment.md, never real
...
Monitor has to infer misbehavior from the raw syscalls themselves (see
ADR-0001), the same way a genuinely rogue agent's own internal logs
can't be trusted either. v0's Monitor only traces process execs, so of
...
MODE=evade is the v4 adversarial test's target: it deliberately tries to
...
def spawn_heartbeat_daemon() -> None:
    ...
    whole *container* once Halt kills it is exactly what the v4
...
def main() -> None:
    ...
    if MODE == "evade":
        print("agent entering evade mode -- attempting to survive graceful "
              ...
              "process dies (the v4 adversarial test)", flush=True)
